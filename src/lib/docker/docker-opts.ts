...
import { isCustomContainerRuntime, isCustomRuntime, hackCustomRuntime } from '../common/model/runtime';
...
export function resolveDockerUser({ runtime, nasConfig }): string {
  ...
  if (process.platform === 'linux') {
    ...
    if (userId === -1 || userId === undefined) {
      userId = hackCustomRuntime(runtime).userId;
    }
    if (groupId === -1 || groupId === undefined) {
      groupId = hackCustomRuntime(runtime).groupId;
    }
  }

  logger.debug(`docker User: ${userId}:${groupId}`);
